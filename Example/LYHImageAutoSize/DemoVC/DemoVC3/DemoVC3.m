...
-(CGFloat )itemHeightAtIndexPath:(NSIndexPath *)indexPath{
     ...
     */
    return [LYHImageAutoSize imageHeightForURL:[NSURL URLWithString:url] layoutWidth:self.layout.itemWidth estimateHeight:200];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:url] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        ...
        [LYHImageAutoSize storeImageSize:image forURL:imageURL completed:^(BOOL result) {
            /** reload */
            if(result) [collectionView lyh_reloadDataForURL:imageURL];
        }];
